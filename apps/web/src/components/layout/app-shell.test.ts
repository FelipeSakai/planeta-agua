...
describe("app shell navigation", () => {
  ...
  it("hides admin-only entries from operators", () => {
    ...
    expect(items).toEqual(["Dashboard", "Nova Venda", "Historico", "Caixa", "Produtos", "Clientes", "Entregadores", "Financeiro"]);
  });

  it("shows all entries to admins", () => {
    const items = getVisibleNavigation("ADMIN").map((item) => item.label);

    expect(items).toEqual(["Dashboard", "Nova Venda", "Historico", "Caixa", "Produtos", "Clientes", "Entregadores", "Estoque", "Financeiro", "Usuarios"]);
  });
