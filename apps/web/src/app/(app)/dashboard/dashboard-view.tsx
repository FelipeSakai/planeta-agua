    ...
    <section className="space-y-6">
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <Panel className="flex min-h-56 flex-col justify-between bg-[var(--hero-surface)] p-6 text-white">
          <div>
            <p className="text-sm font-medium text-white/90">{userName ? `Bom dia, ${userName}` : "Resumo do dia"}</p>
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            <Link
              href="/vendas"
              className="inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] border border-white/30 bg-[var(--hero-action)] px-4 py-2 text-sm font-semibold transition duration-150 hover:bg-[var(--hero-action-hover)] motion-reduce:transition-none"
              style={{ color: "var(--hero-action-foreground)" }}
            >
              Comecar venda
            </Link>
            <Link
              href="/entregas"
              className="inline-flex min-h-10 items-center rounded-[var(--radius-control)] border border-white/35 bg-white/10 px-4 text-sm font-medium text-white transition duration-150 hover:bg-white/20 motion-reduce:transition-none"
            >
              Ver entregas
            </Link>
            <Link
        ...
        <Panel className="p-4">
          <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="text-sm text-[var(--muted)]">Vendas que ainda precisam de confirmacao individual.</p>
            </div>
            ...
            {data.pendingDeliveries.map((delivery) => (
              <li key={delivery.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-[var(--foreground)]">{delivery.customerName ?? "Consumidor"}</p>
                  <p className="text-xs text-[var(--muted)]">
                    {delivery.customerAddress ?? "Endereco nao informado"} · {delivery.driverName ?? "Sem entregador"}
                  </p>
                </div>
                <div className="flex items-center justify-between gap-3 sm:flex-col sm:items-end sm:justify-start">
                  <span className="text-sm font-semibold text-[var(--foreground)]">{formatCentsToBRL(delivery.totalAmountCents)}</span>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <Panel className="p-4">
          <div className="mb-4 flex flex-col gap-1">
            ...
            <p className="text-sm text-[var(--muted)]">Distribuicao das vendas concluidas hoje por forma de pagamento.</p>
          </div>
            ...
            <ul className="divide-y divide-[var(--border)]">
              {data.lowStockProducts.map((product) => (
                <li key={product.id} className="flex items-center justify-between py-3">
                  ...
                  {canOpenStock ? (
                    <Link href="/estoque" className="text-sm font-medium text-[var(--brand)] hover:underline">
                      Reposicao
                    </Link>
                  ) : null}
                </li>
          ...
          <ul className="divide-y divide-[var(--border)]">
            {data.recentSales.map((sale) => (
              <li key={sale.id} className="flex items-center justify-between py-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-[var(--foreground)]">{sale.customerName ?? "Consumidor"}</p>
                  <p className="text-xs text-[var(--muted)]">
                    {paymentMethodLabels[sale.paymentMethod]} · {recentSaleStatusLabels[sale.status]} · {formatTimeOfDay(sale.createdAt)}
                  </p>
                </div>
                <span className="text-sm font-semibold text-[var(--foreground)]">{formatCentsToBRL(sale.totalAmountCents)}</span>
              </li>
            ))}
          </ul>
        ) : (
